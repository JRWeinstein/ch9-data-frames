...
most_pop_2013 <- names_2013$name[names_2013$prop == max(names_2013$prop)]

# Write a function `most_popular_in_year` that takes in a year as a value and 
# returns the most popular name in that year
most_popular_in_year <- function(value) {
  year_sub <- subset(names, names$year == value)
  return(year_sub$name[year_sub$prop == max(year_sub$prop)])
}
...
proportion <- names$prop(names$name == "jennifer")
